...
#funkcja wczytuje dane z pliku .txt
...
#sekwencyjny algorytm zachłanny
...
#funkcja oblicza wartosć dopasowania
...
#funkcja dodaje do populacji wartość dopasowania(liczba błędnych krawędzi)
    ...
    for i in range(10000):
        ...
        if (x:=get_generation(all_colors, population,)):
            ...
            if len(set([i['fit'] for i in population.values()])) == 1:
                ...
                if counter == 1:
                    if a<5:
                        print("stworzona całkowicie nowa populacja")
                        counter = 0
                        a+=1
                        population = create_population(chromosome_size, max_liczba_kolorow, population_size)
                        population = add_fitness(G, population)
                    else:
                        print("wyszukiwanie zakonczone")
                        exit()
            else: 
                sorted_pop = sorted(population, key=lambda x: population[x]['fit'])
                #population = { i:population[v] for i,v in enumerate(sorted_pop[:69] + [sorted_pop[75]] +sorted_pop[95:125]) } #le450_5a
                population = { i:population[v] for i,v in enumerate(sorted_pop[:79] + [sorted_pop[85]] +sorted_pop[105:125]) } #znaleziono 6
                #population = { i:population[v] for i,v in enumerate(sorted_pop[:29] + [sorted_pop[35]] +sorted_pop[59:79]) } 
