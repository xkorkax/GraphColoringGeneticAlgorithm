...
def get_generation(all_colors, population):
    for j in range(30):
        ...
        if((x:=check_fittness(G, child, )) == 0):
            ...
            population[j+49] = {'population': child, 'fit': x}
    return 0
    ...
    for i in range(10000):
        ...
        if (x:=get_generation(all_colors, population,)):
            ...
            if len(set([i['fit'] for i in population.values()])) == 1:
                ...
                population = { i:population[v] for i,v in enumerate(sorted_pop[:29] + [sorted_pop[35]] +sorted_pop[59:79]) } #wywalamy ostatni //dla 124
